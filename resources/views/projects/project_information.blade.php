            ...
            <div class="tab-content">
                ...
                <div class="tab-pane fade {{ $activeTab == 'tasks' ? 'active show' : '' }}" id="navs-top-tasks" role="tabpanel">
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        @php
                            $taskView = getUserPreferences('tasks', 'default_view');
                        @endphp
                        <div>
                             <div class="btn-group" role="group" aria-label="Task View">
                                <button type="button" class="btn btn-outline-primary {{ $taskView == 'tasks/list' || $taskView == 'tasks' || $taskView == '' ? 'active' : '' }}" id="btn-list-view" onclick="toggleTaskView('list')" data-bs-toggle="tooltip" data-bs-placement="left" data-bs-original-title="<?= get_label('list_view', 'List View') ?>"><i class='bx bx-list-ul'></i></button>
                                <button type="button" class="btn btn-outline-primary {{ $taskView == 'tasks/draggable' ? 'active' : '' }}" id="btn-kanban-view" onclick="toggleTaskView('kanban')" data-bs-toggle="tooltip" data-bs-placement="left" data-bs-original-title="<?= get_label('kanban_view', 'Kanban View') ?>"><i class='bx bxs-grid'></i></button>
                            </div>
                        </div>
                    ...
                    $clients = $project->clients;
                    ?>
                    <div id="task-list-view" class="{{ $taskView == 'tasks/list' || $taskView == 'tasks' || $taskView == '' ? '' : 'd-none' }}">
                        <x-tasks-card :tasks="$tasksCount" :id="$id" :users="$users" :clients="$clients" :emptyState="0" />
                    </div>
                    <div id="task-kanban-view" class="{{ $taskView == 'tasks/draggable' ? '' : 'd-none' }}">
                        <x-tasks-kanban-board :tasks="$project->tasks" :statuses="$statuses" />
                    </div>
                    <script>
                        function toggleTaskView(view) {
                            if (view === 'list') {
                                $('#task-list-view').removeClass('d-none');
                                $('#task-kanban-view').addClass('d-none');
                                $('#btn-list-view').addClass('active');
                                $('#btn-kanban-view').removeClass('active');
                                saveViewPreference('tasks/list');
                            } else {
                                $('#task-list-view').addClass('d-none');
                                $('#task-kanban-view').removeClass('d-none');
                                $('#btn-list-view').removeClass('active');
                                $('#btn-kanban-view').addClass('active');
                                saveViewPreference('tasks/draggable');
                            }
                        }

                        function saveViewPreference(view) {
                            $.ajax({
                                url: '/save-tasks-view-preference',
                                method: 'PUT',
                                data: {
                                    _token: '{{csrf_token()}}',
                                    view: view
