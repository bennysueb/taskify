...
<div class="container-fluid">
    ...
    <div class="card mt-4">
        <div class="card-body">
            <h5 class="card-title"><?= get_label('test_email_configuration', 'Test Email Configuration') ?></h5>
            <form action="{{url('settings/email/test')}}" class="form-submit-event" method="POST">
                @csrf
                <div class="row">
                    <div class="mb-3 col-md-12">
                        <label for="test_email" class="form-label"><?= get_label('test_email_address', 'Test Email Address') ?> <span class="asterisk">*</span></label>
                        <input class="form-control" type="email" id="test_email" name="test_email" placeholder="<?= get_label('enter_email_to_test', 'Enter email to receive test mail') ?>" required>
                    </div>
                    <div class="mt-2">
                        <button type="submit" class="btn btn-info me-2"><?= get_label('send_test_email', 'Send Test Email') ?></button>
                    </div>
                </div>
            </form>
        </div>
