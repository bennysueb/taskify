                                    ...
                                    <div class="col-md-{{ $task->note ? '7' : '6' }} mb-3">
                                        ...
                                        <div class="d-flex align-items-center">
                                            <select
                                                ...
                                                data-type="task">
                                                @php
                                                    $targetType = $task->parent_id ? 'subtask' : 'task';
                                                @endphp
                                                @foreach ($statuses as $status)
                                                    @php
                                                        $statusTypes = explode(',', $status->type);
                                                    @endphp
                                                    @if (in_array($targetType, $statusTypes))
                                                        @php
                                                            $disabled = canSetStatus($status, $targetType) ? '' : 'disabled';
                                                        @endphp
                        ...
                        <div id="navs-top-sub-task"
                            ...
                            <div class="d-flex justify-content-between align-items-center mb-4">
                                ...
                                <div class="btn-group" role="group" aria-label="Task View">
                                    <button type="button"
                                        class="btn btn-outline-primary {{ $taskView == 'tasks/list' || $taskView == 'tasks' || $taskView == '' ? 'active' : '' }}"
                                        id="btn-list-view" onclick="toggleTaskView('list')" data-bs-toggle="tooltip"
                                        data-bs-placement="left"
                                        data-bs-original-title="<?= get_label('list_view', 'List View') ?>"><i
                                            class='bx bx-list-ul'></i></button>
                                    <button type="button"
                                        class="btn btn-outline-primary {{ $taskView == 'tasks/draggable' ? 'active' : '' }}"
                                        id="btn-kanban-view" onclick="toggleTaskView('kanban')" data-bs-toggle="tooltip"
                                        data-bs-placement="left"
                            ...
                            <div id="task-list-view"
                                class="{{ $taskView == 'tasks/list' || $taskView == 'tasks' || $taskView == '' ? '' : 'd-none' }}">
                            ...
                            <div id="task-kanban-view" class="{{ $taskView == 'tasks/draggable' ? '' : 'd-none' }}">
                                <x-tasks-kanban-board :tasks="$task->subtasks" :statuses="$statuses" />
                            </div>
