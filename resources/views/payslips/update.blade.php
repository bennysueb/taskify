            ...
            <form action="{{url('payslips/update')}}" class="form-submit-event" method="POST">
                ...
                <div class="row">
                    ...
                    <div class="mb-3 col-md-4">
                        ...
                        <select class="form-select js-example-basic-multiple" name="division_id" data-placeholder="<?= get_label('type_to_search', 'Type to search') ?>" data-allow-clear="true">
                            <option value=""><?= get_label('select_division', 'Select Division') ?></option>
                            @foreach ($divisions as $division)
                                <option value="{{ $division->id }}" {{ $payslip->division_id == $division->id ? 'selected' : '' }}>{{ ucfirst($division->title) }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-3 col-md-4">
