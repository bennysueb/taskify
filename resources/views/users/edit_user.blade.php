            ...
            <form action="{{url('users/update_user/' . $user->id)}}" class="form-submit-event" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    ...
                    <div class="mb-3 col-md-6">
                        ...
                        <select class="form-select text-capitalize js-example-basic-multiple" id="division_id" name="division_id" data-placeholder="<?= get_label('Please select', 'Please select') ?>" data-allow-clear="true">
                            <option></option>
                            @foreach ($divisions as $division)
                            <option value="{{$division->id}}" {{ $user->division_id == $division->id ? "selected" : "" }}>{{ ucfirst($division->title) }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-3 col-md-6">
