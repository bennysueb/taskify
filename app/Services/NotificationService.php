    ...
    private function sendBirthdayWishes($today)
    {
        ...
        foreach ($users as $user) {
            ...
            if ($user->telegram_chat_id) {
                $data = [
                    'type' => 'birthday_wish',
                    'birthday_count' => '', // Valid logic to calculate age needed
                    'ordinal_suffix' => ''
                ];
                sendTelegramNotification($user, $data);
            }
        }
    }
    ...
    private function sendAnniversaryWishes($today)
    {
        ...
        foreach ($clients as $client) {
            ...
            if ($client->telegram_chat_id) {
                 $data = [
                    'type' => 'work_anniversary_wish', // Assuming this type exists in get_message
                     'work_anniversary_count' => '',
                    'ordinal_suffix' => ''
                ];
                sendTelegramNotification($client, $data);
            }
        }
    }
}
