...
class UserController extends Controller
{
    ...
    public function create()
    {
        ...
        return view('users.create_user', ['roles' => $roles, 'divisions' => $divisions]);
    }
    ...
    public function edit_user($id)
    {
        ...
        $roles = Role::where('guard_name', 'web')->get();
        $divisions = Division::where('workspace_id', getWorkspaceId())->get();
        return view('users.edit_user', ['user' => $user, 'roles' => $roles, 'divisions' => $divisions]);
    }
        ...
        $users = $users->select('users.*')
            ...
            ->through(
                function ($user) use ($workspace, $canEdit, $canDelete, $canManageProjects, $canManageTasks, $mainAdminId, $guardName, $authUserId) {
                    ...
                    $projectsBadge = '<span class="badge rounded-pill bg-primary">' . (isAdminOrHasAllDataAccess('user', $user->id) ? $workspace->projects->count() : $user->projects->count()) . '</span>';
                    ...
                    $tasksBadge = '<span class="badge rounded-pill bg-primary">' . (isAdminOrHasAllDataAccess('user', $user->id) ? $workspace->tasks->count() : $user->tasks->count()) . '</span>';
                    if ($canManageTasks) {
                    ...
                    return [
                        ...
                        'division' => $user->division ? ucfirst($user->division->title) : '-',
                        'email' => $user->email,
                        'phone' => $phone,
