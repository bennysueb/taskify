...
class SettingsController extends Controller
{
    ...
    public function send_test_email(Request $request)
    {
        ...
        try {
            // Force reload settings from DB to ensure we use the latest values
            $emailSettings = get_settings('email_settings');
            ...
            \Mail::raw('This is a test email to verify your SMTP settings. If you received this, your email configuration is correct.', function ($message) use ($request) {
                $message->to($request->test_email)
                    ->subject('Test Email from ' . config('app.name'));
            });

            return response()->json(['error' => false, 'message' => 'Test email sent successfully.']);
        } catch (\Exception $e) {
            $errorMessage = $e->getMessage();
            if (str_contains($errorMessage, 'Connection to "smtp.gmail.com:465" has been closed unexpectedly')) {
                $errorMessage .= ' (Hint: Try using port 587 with TLS, or ensure SSL is selected for port 465. Also check if your server firewall allows outgoing connections on this port.)';
            }
            return response()->json(['error' => true, 'message' => 'Failed to send test email: ' . $errorMessage]);
        }
    }
    ...
    public function store_telegram_settings(Request $request)
    {
        $request->validate([
        ...
        // Check if the setting exists
        $existingSetting = Setting::where('variable', 'telegram_settings')->first();
...
<p><small>Visit us at: {SITE_URL}</small></p>'
    ...
    public function testNotificationSettings(Request $request)
    {
        ...
        if ($type == 'slack') {
            ...
            $recipient = (object) [
                'telegram_chat_id' => $request->input('telegramChatId')
            ];
        } else {
            $recipient = (object) [
        ...
        try {
            if ($type === 'sms') {
                ...
                $response = sendTelegramNotification($recipient, null, $message);
            } else {
