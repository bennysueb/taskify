...
class ContractsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('contracts.list', ['contracts' => $contracts, 'divisions' => $divisions]);
    }
    ...
    public function list()
    {
        ...
        $contracts = Contract::select(
            ...
            ->leftJoin('users as employees', 'contracts.user_id', '=', 'employees.id');
        ...
        if (!empty($user_ids)) {
            $contracts = $contracts->whereIn('contracts.user_id', $user_ids);
        }
        if (!empty($type_ids)) {
        ...
        $contracts = $contracts->orderBy($sort, $order)
            ...
            ->through(function ($contract) use ($canEdit, $canDelete, $canCreate) {
                ...
                return [
                    ...
                    'user' => $contract->user ? formatUserHtml($contract->user) : '',
                    'contract_type' => $contract->contract_type,
                    'division' => $contract->division ? ucfirst($contract->division->title) : '-',
