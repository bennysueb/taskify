...
if (!function_exists('processNotifications')) {
    function processNotifications($data, $recipients)
    {
        ...
        if (!empty($recipients)) {
            ...
            $telegramNotificationTemplate = getNotificationTemplate($data['type'], 'telegram');
            foreach ($recipients as $recipient) {
                ...
                if ($recipientModel) {
                    ...
                    if (!$telegramNotificationTemplate || ($telegramNotificationTemplate->status !== 0)) {
                        if (
                            (is_array($enabledNotifications) && empty($enabledNotifications)) || (
                                is_array($enabledNotifications) && (
                                    in_array('telegram_' . $data['type'] . '_assignment', $enabledNotifications) ||
                                    in_array('telegram_' . $data['type'], $enabledNotifications)
                                )
                            )
                        ) {
                            try {
                                sendTelegramNotification($recipientModel, $data);
                            } catch (\Exception $e) {
                            }
                        }
                    }
...
if (!function_exists('sendTelegramNotification')) {
    function sendTelegramNotification($recipient, $itemData = NULL, $message = NULL)
    {
        $telegram_settings = get_settings('telegram_settings');
        // Treat telegram messages same as whatsapp for content generation
        $msg = $itemData ? get_message($itemData, $recipient, 'telegram') : $message;
        $botToken = $telegram_settings['telegram_bot_token'];
...
if (!function_exists('get_message')) {
    ...
    {
        // Treat telegram same as sms/whatsapp for template retrieval
...
if (!function_exists('canSetStatus')) {
    function canSetStatus($status, $type = null)
    {
        $user = getAuthenticatedUser();
        ...
        if ($type !== null) {
            $statusTypes = explode(',', $status->type);
...
if (!function_exists('formatCandidate')) {
    function formatCandidate($candidate)
    {
        return [
            ...
            'division' => $candidate->division ? ucfirst($candidate->division->title) : null,
        ...
        return [
            ...
            [
                ...
                'submenus' => [
                    ...
                    [
                        'id' => 'divisions',
                        'label' => get_label('divisions', 'Divisions'),
                        'url' => url('/divisions'),
                        'class' => 'menu-item' . (Request::is('divisions*') ? ' active' : ''),
                        'show' => $user->can('manage_divisions') ? 1 : 0,
                    ],
                ]
            ],
            [
    ...
    if (!function_exists('formatPayslip')) {
        function formatPayslip($payslip)
        {
            ...
            return [
                ...
                'division' => $payslip->division ? ucfirst($payslip->division->title) : null,
                'division_id' => $payslip->division_id,
                'month' => $payslip->month,
        ...
        try {
            ...
            while ($attempt < $maxRetries) {
                try {
                    ...
                    // Set expiry if keys are new (ttl is handled by cache configuraton, but good to be explicit for minute/day)
                    // Simplified: just update values. Expiry logic can be more complex if needed.
                    return [
                        'error' => false,
                    ...
                    if ($attempt >= $maxRetries) {
                        ...
                        return [
                            'error' => true,
                            'message' => 'An error occurred while generating the description using Gemini API.',
                        ];
                    }
                    // Wait before retrying
                    $retryDelay = $settings['retry_delay'] ?? 1;
            ...
            Log::error('Gemini Configuration Error', [
                'message' => $e->getMessage(),
            ]);
            return [
                'error' => true,
